...
<table cellpadding="0" cellspacing="0" border="1" width="1000">
	...
	<tr>
		<td align="right" class="evencolumn"> Оплачено/Завдаток </td>
		<td align="left" class="oddcolumn">
...
function updatePage() {
	...
	function add_skin2(j, height, width, gprice, count){
		var s = '<input type="hidden" name="sellings['+j+'][skin][width]" value="'+width+'">';
			...
			s+= '<table width="100%"><tr><td width="40px;">&nbsp;</td><td style="font-style: italic; font-weight:bold;">Плівка антитравматична ('+count*height*width/1000000+') </td>';
	...
	function add_skin(){
		for (var j=0; j<=i; j++){
			...
			add_skin2(j, height, width, 0, count);
	...
	function add_selling2(height, width, count, good_type, label, price){
		...
		+'\n<td class="column">'+label+' ('+count*width*height/1000000+')</td>'
		...
		+'\n<td class="column" width="60px;"> кі-сть  <strong>'+count+'</strong><input  value="'+count+'" id="count'+i+'" type="hidden" name="sellings['+i+'][count]" value="1"/></td>'
