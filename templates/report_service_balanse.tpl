	...
	<td colspan='2'>
		<table cellpadding="0" cellspacing="0" border="1" width="100%">
			...
			<tr {if (!$row.date)}id="1" onclick="change_visibility(this);"{else}class="hiddenrow"{/if}>
				...
				<td align="center" class="oddcolumn"><a href="index.php?action=edit_client&id={$row.client}">{$row.client_name}</a>&nbsp;</td>
				<td align="center" class="evencolumn"><a href="index.php?action=edit_order&id={$row.order_id}">{$row.amount|number_format:0:",":" "}&nbsp;</a> </td>
				{/if}
				{/if}
			</tr>
			{/foreach}
